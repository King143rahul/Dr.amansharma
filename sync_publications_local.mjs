...
(async () => {
  ...
  const insertedData = enrichedPublications; const insertError = null; // Fetch IDs and titles to map order
  ...
  insertedData.forEach(p => publicationOrder.push(p.title));
  ...
  const orderFilePath = path.join(dataDir, 'publications_with_abstracts.json');
  fs.writeFileSync(orderFilePath, JSON.stringify(enrichedPublications, null, 2));
  
  console.log(`Success! Wrote ${publicationOrder.length} publication IDs to src/data/publications_with_abstracts.json`);
